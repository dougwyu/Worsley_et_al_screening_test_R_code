---
title: "Figure S9 Growth rate experiment using Softbugs isolated from _Acromyrmex_ ants (resident non-producers)"
author: "Douglas Yu"
date: "18 May 2020"
...
```{r}
datafile <- "Figure_S9_Growth_rate_experiment_DataSheet_20180303.xlsx"
...
assay3 <- assay3 %>% select(Ps.genotype, Invader, Inv.strain, Ps.strain, Plate, Invasion.score) %>% arrange(Ps.genotype, Invader, Inv.strain, Ps.strain, Plate)
...
(q <- p + ylim(0, 2.5) # to have same y-range as Figure 1A
	+ geom_boxplot(size = .5, outlier.shape = NA, width = 0.3, aes(fill = Invader)) 
	+ geom_jitter(width = 0.05, cex = 0.7) 
