---
title: "Figure 4 Growth rate experiment"
author: "Douglas Yu"
date: "18 May 2020"
...
```{r}
datafile <- "Figure_4_Growth_rate_experiment_DataSheet_20180302.xlsx"
...
assay1 <- assay1 %>% select(-Input.order)
...
assay2 <- assay2 %>% select(-Input.order)
...
assay3 <- assay3 %>%
  select(Ps.genotype, Invader, Inv.strain, Ps.strain, Plate, Invasion.score) %>%
  arrange(Ps.genotype, Invader, Inv.strain, Ps.strain, Plate)
...
(q <- p + ylim(0, 2.5)
  ...
  + labs(y = expression("Growth score:  Mean colony area " ~ (cm^{
    2
  })))
...
# the published figure was edited outside of R to add the grey panel and x-axis labelling. 
```
...
This dataset is used to choose strains for the pairwise competition experiment (Fig 6)
...
assay4.2 <- assay3 %>%
  group_by(Ps.genotype, Invader, Inv.strain) %>%
  summarise(median.Invasion.score = round(median(Invasion.score), 2))

assay4.2 <- assay4.2 %>% arrange(Ps.genotype, Invader, median.Invasion.score)

assay4.3 <- assay3 %>%
  group_by(Ps.genotype, Ps.strain, Invader, Inv.strain) %>%
  summarise(median.Invasion.score = round(median(Invasion.score), 2))
...
1. Pseudonocardia genotype: `Ps.genotype` (Ps1, Ps2).
...
E1 <- rstandard(M1)
plot(E1 ~ assay3$Ps.strain)
abline(0, 0) # T,P,Q strains either mostly above or below 0, demonstrating correlated residuals for some strains
plot(E1 ~ assay3$Inv.strain)
abline(0, 0) # 11,14,16,17,18,19,20,4,5,9,10 either mostly above or below 0, demonstrating correlated residuals for some strains. recall that 11-20 are softbugs and 1-10 are _Streptomyces_
rm(M1)
rm(E1)
```
...
Random factors:
```{r}
plot(assay3$Ps.strain, resid(M2), xlab = "Ps.strain", ylab = "Residuals")
abline(0, 0)
plot(assay3$Inv.strain, resid(M2), xlab = "Inv.strain", ylab = "Residuals")
abline(0, 0)
plot(assay3$Plate, resid(M2), xlab = "Inv.strain", ylab = "Residuals")
abline(0, 0)
```

The correlated residuals have gone away, as expected, although Inv.strain 18 has greater variance than the other strains. 

### Effect of Ps1 vs Ps2

From the Andersen et al.'s (2013) results, we hypothesised that Ps2 would be a stronger growth suppressor than is Ps1.

Remove the Control level and test again for a significant main effect of Ps.genotype (Ps1 vs Ps2).
```{r}
assay4 <- assay3 %>% filter(Ps.genotype != "Control")
